...
function initRequireJs() {
    require.config({
        ...
        map: {
            '*': {
                'css': window.BASE_PATH + 'plugins/require-css/0.1.10/css.js',
                'text': window.BASE_PATH + 'plugins/requirejs-text/2.0.15/text.js'
            }
        },
        shim: {
            'jquery': {exports: "$"}
        ...
        paths: {
            "jquery": [window.BASE_PATH + "plugins/jquery/1.10.2/jquery.min"],
            "logger": [window.BASE_PATH + "admin/logger"],
            "authorize": [window.BASE_PATH + "admin/commons/authorize"], //权限管理
            "request": [window.BASE_PATH + "plugins/tools/request"], //ajax请求工具
            "miniui-tools": [window.BASE_PATH + "plugins/miniui/tools"],
            "message": [window.BASE_PATH + "plugins/miniui/message"],
            "ace": [window.BASE_PATH + "plugins/script-editor/ace"],
            "art-template": [window.BASE_PATH + "plugins/template/art-template"],
            "script-editor": [window.BASE_PATH + "plugins/script-editor/script-editor"],
            "plugin": [window.BASE_PATH + "plugins"]
        }
    });
}
...
function importMiniui(callback) {
    function doImport() {
        ...
        function loadMini() {
            ...
            $.ajax({
                url: window.BASE_PATH + "plugins/miniui/miniui.js",
                async: false,
                cache: true,
                dataType: "script",
                success: callback,
