        ...
        return [
            {
                ...
                createEditor: function (component, text, value) {
                    var html;
                    if (window.optionalNames) {
                        html = $("<select lay-search lay-filter='editor-name'>");
                        $(window.optionalNames).each(function () {
                            var option = $("<option >");
                            ...
                            option.attr("comment",this.comment);
                            html.append(option);
                        });
                        layui.form.on("select(editor-name)",function (e) {
                            var comment = $(e.elem).find("[value="+e.value+"]").attr("comment");
                            component.setProperty("comment",comment);
                            component.setProperty("name",e.value);
                            console.log(comment);
                        });
                    } else {
                        html = $("<input type='text' class='layui-input' />");
                        html.val(value);
                        html.on("change", function () {
                            component.setProperty("name", html.val());
                        }).on("keyup", function () {
                            component.setProperty("name", html.val());
                        });
                    }
                ...
                this.un("propertiesChanged")
                    .on('propertiesChanged', function (name, value) {
                        if (name === 'comment') {
                            ...
                            container.find("textarea").attr(name, value);
                        }
                    });
                return container;
            };
